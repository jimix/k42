 ...
 *
 * $Id: FSFileKFSK42.C,v 1.5 2004/05/06 19:52:50 lbsoares Exp $
 *****************************************************************************/
...
FSFileKFS::createServerFileBlock(ServerFileRef &ref)
{
    ...
    rc = ServerFileBlockKFS::Create(ref, this);
	...
	FileLinux::Stat *status /* = NULL */)
{
    ...
    *entryInfo = new FSFileKFS(globals, &otokID, lso->getLocalRecordMap());
    ...
    ((FSFileKFS *)*entryInfo)->reValidateFSFile(status);
    if ((status->st_nlink > 1) && status->isFile()) {
	MultiLinkManager::SFHolder *href;
	globals->multiLinkMgr.acquireLock();
	if (globals->multiLinkMgr.locked_search((uval)(*entryInfo), href)==0) {
	    // create server file and add
	    ServerFileBlockKFS::Create(ref, *entryInfo);
	    href = MultiLinkManager::AllocHolder(ref);
	    globals->multiLinkMgr.locked_add((uval)(*entryInfo), href);
	} else {
	    ref = href->fref;
	}
	lock = &href->lock;
