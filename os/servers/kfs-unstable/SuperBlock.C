 ...
 *
 * $Id: SuperBlock.C,v 1.6 2004/05/06 19:52:49 lbsoares Exp $
 *****************************************************************************/
/******************************************************************************
 *
...
#else
    setRecMap((uval32)(GlobalRecordMap::locationAlloc(globals)));
#endif
...
SuperBlock::createRootDirectory(void)
{
...
#if 0 // tentative, while debugging
    err_printf("Before first flush\n");
    // These markDirty/flush are only necessary until BlockCache works
    // properly in K42. (This code is not being used in Linux right now).
    globals->recordMap->markDirty();
...
#else
    if (globals->recordMap->allocRecord(OT_LSO_DIR_EMB, &rootTokID) < 0) {
#endif
	err_printf("KFS::formatKFS() Error allocating directory token!\n");
    }
    setRootLSO(rootTokID);
    //    rootTok.setID(rootTokID);

#if 0 // tentative, while debugging
    err_printf("Before second flush\n");
    // These markDirty/flush are only necessary until BlockCache works
    // properly in K42. (This code is not being used in Linux right now).
    globals->recordMap->markDirty();
    // sync the disk metadata
    ...
    lso = (LSOBasicDir *)globals->recordMap->getObj(&rootTokID);
    lso->initAttribute(S_IFDIR | (0755 & ~S_IFMT), 0, 0);

    // now create the '.' and '..' directories
    lso->createEntry(".", 1, &rootTokID, (0755 & ~S_IFMT), 0);
    lso->createEntry("..", 2, &rootTokID, (0755 & ~S_IFMT), 0);
...
#define SB_PRINT16(name, v) err_printf("\t " name " CPU sees bits: 0x%x", v); \
    ...
    err_printf("\t rootLSO.id: CPU 0x%llx ", TE64_TO_CPU(rootLSO.id));
    PRINT_TE64(rootLSO.id, 1);
