 ...
 *
 * $Id: BlockCacheK42.C,v 1.37 2005/08/24 14:55:38 dilma Exp $
 *****************************************************************************/
...
BlockCacheEntryK42::markDirty()
{
    ...
    if (!isDirty) {
	isDirty = 1;
	isEmpty = 0; // no need to read it again if becomes clean
	flushTime = Scheduler::SysTimeNow() + 
	    Scheduler::TicksPerSecond()*FLUSH_TIME;
	TraceOSDiskKFSMetaDataDirty(blkno, (uval) d);
    }
    return 0;
}
...
BlockCacheEntryK42::markClean()
{
    ...
    if (isDirty) {
	TraceOSDiskKFSMetaDataClean(blkno, (uval) d);
    }
...
BlockCacheEntryK42::readData()
{
    DBCE("readData", blkno);
    if (blkno) {
	/* isDirty set means that we have already written to the block,
	 * isEmpty set that we have already read it */
	if (isEmpty) {
...
{
    tassertWrn(isDirty, "flushing clean entry in blockcache\n");
    tassertMsg(blkno, "?");
...
BlockCacheK42::BlockCacheK42(Disk *disk, KFSGlobals *g)
	: d(disk), globals(g), te(g), doingFlush(0)
{
    if (globals->isSyncMetaDataOn()) {
	TimerEventBC::ScheduleEvent(&te);
    }
#if 0 // trying to make sure the timer events are being triggered
...
BlockCacheK42::markDirty(BlockCacheEntry *entry)
{
	...
	if (Scheduler::SysTimeNow() > lastTimerCreationTime
	    + Scheduler::TicksPerSecond()*
	                   (TimerEventBC::DEFAULT_INTERVAL+14)) {
	    tassertWrn(0, "KFS:It seems the events stopped being generated!!!\n");
	    TimerEventBC *t = new TimerEventBC(globals);
...
BlockCacheK42::flush(uval checkFlushTime /* = 1 */)
{
	...
	if (entry->getIsDirty()) {
		...
		//	   _SGETPID(DREFGOBJ(TheProcessRef)->getPID()));
		TraceOSDiskKFSMetaDataFlush(entry->getBlockNumber(),
					  (uval) globals->super);
		entry->flush();
	    } else {
...
/* virtual */ SysStatus
BlockCacheK42::readBlock(uval32 pblkno, char *buffer,
			 void *cont /* = NULL */)
{
...
/* virtual */ SysStatus
BlockCacheK42::writeBlock(uval32 pblkno, char *buffer,
			  void *cont /* = NULL */)
{
...
BlockCacheK42::sync()
{
    TraceOSDiskKFSSyncBlockCache((uval) globals->super);
