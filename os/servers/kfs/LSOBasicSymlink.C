 ...
 *
 * $Id: LSOBasicSymlink.C,v 1.9 2004/03/07 00:42:39 lbsoares Exp $
 *****************************************************************************/
...
LSOBasicSymlink::LSOBasicSymlink(ObjTokenID otokID, FSFileKFS *f, KFSGlobals *g)
    : LSOBasic(otokID, f, g) { }
			       ...
			       uval uid, uval gid)
{
	...
	for (uval i = 0; i < nrec; i++) {
	    ...
	    uval size;
	    ObjTokenID tmp;
	    rc = globals->recordMap->allocRecord(OT_SYMLINK_EXT, &tmp);
	    ...
	    globals->recordMap->setRecord(symlinkStruct->getObjTokenID(i),
					  cutValue);
	}
...
//	buffer[linkSize] = 0;
	...
	for (uval i = 0; i < nrec; i++) {
	    ...
	    globals->recordMap->getRecord(symlinkStruct->getObjTokenID(i),
					  cutValue);
...
LSOBasicSymlink::deleteFile()
{
	...
	if (linkSize <= MED_SYMLINK_SIZE) {
		...
		globals->recordMap->freeRecord(&tmp);
	    }
	} else if (linkSize <= OS_BLOCK_SIZE) {
...
LSOBasicSymlink::clone(ObjTokenID otokID, FSFileKFS *f)
{
    LSOBasicSymlink *lso = new LSOBasicSymlink(otokID, f, globals);
    lso->init();
