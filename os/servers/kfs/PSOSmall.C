/***************************************************************************
 * Copyright (C) 2003, 2004 Livio B. Soares (livio@ime.usp.br)
 * Licensed under the LGPL
 *
 * Changes/additions by the K42 group.
 *
 * $Id: PSOSmall.C,v 1.36 2005/07/05 16:52:17 dilma Exp $
 **************************************************************************/
 ...
 */
PSOSmall::PSOSmall(ObjTokenID otokID, FSFileKFS *f, PSOBase *p, KFSGlobals *g)
    : PSOBase(otokID, f, g), llpso(p)
{
    // read the PSO record
...
PSOSmall::~PSOSmall()
{
    KFS_DPRINTF(DebugMask::PSO_SMALL,
		"PSOSmall::~PSOSmall() id=%u IN\n", id.id);
    KFS_DPRINTF(DebugMask::PSO_SMALL,
		"PSOSmall::~PSOSmall() id=%u OUT\n", id.id);
}
...
PSOSmall::truncate(uval32 lblkno)
{
	...
	if (depth > 1) {
	    blocks[i+1] = globals->blkCache->getBlockRead(
		    TE32_TO_CPU(dblk[offsets[i]]));
	    dblk = (uval32 *)blocks[i+1]->getData();
	}
    }

    tassertMsg(depth >= 1 && depth <= 4, "depth %d\n", depth);
 ...
 */
sval
PSOSmall::readBlock(uval32 lblkno, char *buffer,
 ...
 */
sval
PSOSmall::readBlockPhys(uval32 lblkno, char *buffer,
			PSOBase::AsyncOpInfo *cont /* = NULL */)
{
    sval rc;
    uval32 dblkno;
...
BlockCacheEntry *
PSOSmall::readBlockCache(uval32 b)
{
 ...
 */
SysStatus
PSOSmall::writeBlock(uval32 lblkno, char *buffer,
		     PSOBase::AsyncOpInfo *cont /* = NULL */)
{
    sval rc;
    uval32 dblkno;
    ...
    if (!dblkno) {
	passertMsg(0, "PSOSmall::writeBlock() This shouldn't happen, no block!"
		   " %u %u\n", dblkno, lblkno);
	memset(buffer, 0, OS_BLOCK_SIZE);
	// FIXME: deal with continuation, return proper error
	return -1;
    }
...
PSOSmall::locked_flush()
{
	...
	globals->recordMap->setRecord(id, recordBuf);
    }
}
...
PSOSmall::clone(ObjTokenID otokID, FSFileKFS *f)
{
    KFS_DPRINTF(DebugMask::PSO_SMALL, "PSOSmall::clone\n");
    ServerObject *pso = new PSOSmall(otokID, f, llpso, globals);
 ...
 */
SysStatus
PSOSmall::locationAlloc(ObjTokenID _id)
{
    memset(recordBuf, 0, KFS_RECORD_SIZE);
    globals->recordMap->setRecord(_id, recordBuf);
