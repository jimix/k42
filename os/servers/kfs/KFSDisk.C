 ...
 *
 * $Id: KFSDisk.C,v 1.12 2004/11/23 22:44:08 dilma Exp $
 *****************************************************************************/

#include <kfsIncs.H>
#include <io/DiskClientRO.H>
#include <io/DiskClientAsyncRO.H>
...
KFSDisk::init(char *dname, uval readOnly)
{
	...
	rc = DiskClientAsyncRO::Create(adcr, dname, CallBack);
	tassertMsg(_SUCCESS(rc), "? rc 0x%lx\n", rc);
    }
    else {
	rc = DiskClient::Create(dcr, dname);
	tassertMsg(_SUCCESS(rc), "? rc 0x%lx\n", rc);
	rc = DiskClientAsync::Create(adcr, dname, CallBack);
	tassertMsg(_SUCCESS(rc), "? rc 0x%lx\n", rc);
    }
 ...
 */
SysStatus
KFSDisk::aReadBlock(uval blkno, char *buf, PSOBase::AsyncOpInfo *cont)
{
    return DREF(adcr)->aReadBlockPhys(blkno, (uval)buf, (uval) cont);
}
 ...
 */
SysStatus
KFSDisk::aWriteBlock(uval blockno, char *buf, PSOBase::AsyncOpInfo *cont)
{
    return DREF(adcr)->aWriteBlockPhys(blockno, (uval)buf, (uval) cont);
}
...
/* static */ void
KFSDisk::CallBack(uval rc, uval args)
{
    PSOBase::AsyncOpInfo *cont = (PSOBase::AsyncOpInfo *) args;
