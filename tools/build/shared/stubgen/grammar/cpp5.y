 ...
 *
 * $Id: cpp5.y,v 1.22 2004/01/30 22:00:03 aabauman Exp $
 *****************************************************************************/

    /* Copyright (C) 1989-1991, 2000 James A. Roskind, All rights reserved.
...
#include <stdio.h>
#include <stdarg.h>
#include <string.h>		/* strdup, strtok, strcpy, ... */
#include "cpp5.h"
...
int warnings = 0;        /* warnings aren't shown by default */
...
char ppctype     = 'S';
char in_ppc_call = NO_CALL;


#define MAX_PARAMS 32
int   num_parms = 0;           /* number of parameters   */
int   type_mask[MAX_PARAMS];   /* type modification list */
char* bufpar_spec[MAX_PARAMS];
int   expect_type = 0;         /* to be set when we expect type in sig */
int   not_type = 0;         /* to be set when we expect type in sig */
char *func_name;
char *class_name;
char *type_name;
char *parent_name;
...
#define XA_DEFAULT  "__xa(processOnly)"
...
/* different appearances the buffer specification parts can have */
#define IS_NUM 0
#define IS_STR 1
#define IS_VAR 2

int   stypemask;         /* type modification for current parameter     */
char *sbufspec;          /* buffer specification for current parameter  */

int   num_sizearr_in;    /* how many slots do we need in the size array */
int   num_sizearr_out;   /* how many slots do we need in the size array */

char* buferrstr[] = {
    "",
    "Constant in/out bufsize[s] exceeds maximum constant bufsize",
    "Both in and out spec must be of <*>"
};

char* SEPS = " \t:)";

void fixbufspec(char *yytext, int kind)
{
    int   n,err;
    char* ptr = yytext;
    char  newbufspec[100];
    ...
    int   what[3]  = { 0, 0, 0 };
    int   value[3] = { 0, 0, 0 };
    int   szarray_in_idx  = -1; /* indexes into the size array */
    int   szarray_out_idx = -1; /* - " -                       */

    strcpy(yytextbak,yytext);  /* need a bak for error message */

    while (*ptr && (*ptr != '(')) ++ptr;
    ptr++;
    newbufspec[0] = '\0';
    n = 0;
    words[n] = strtok(ptr,SEPS);
    while(words[n] != NULL) {
	if      (words[n][0] == '*')                   what[n] = IS_STR;
	else if (sscanf(words[n],"%d",&value[n]) == 1) what[n] = IS_NUM;
	else                                           what[n] = IS_VAR;
	whatmask = whatmask*10 + what[n];
	strcat(newbufspec,words[n]);
	strcat(newbufspec,":");
	n++;
	words[n] = strtok(NULL,SEPS);
    }



    err = 0;
    /* now verify whether this is a valid buffer specification */
    switch (kind) {
    case 0: /* __inbuf */
	whatmask *= 100; /* got to shift over */
	strcat(newbufspec,"?:?:");
	if (what[0] == IS_STR) { szarray_in_idx = num_sizearr_in++; }
	break;
    case 1: /* __outbuf */
	if ((what[0] == IS_NUM) && (what[1] == IS_NUM)
		 && (value[0] > value[1]))                         err = 1;

	tmpbufspec[0] = '?'; tmpbufspec[1] = ':';
	strcpy(&tmpbufspec[2],newbufspec);
	strcpy(newbufspec,tmpbufspec);
	if (what[0] == IS_STR) {szarray_out_idx = num_sizearr_out++; }
	break;

    case 2: /* __inoutbuf */
	if      ((what[0] == IS_STR) && (what[1] != IS_STR))       err = 2;
	else if ((what[0] == IS_NUM) && (what[1] == IS_NUM) &&
		 (what[2] == IS_NUM)  &&
		 ((value[2] < value[0]) || (value[2] < value[1]))) err = 1;

	if (what[0] == IS_STR) { szarray_in_idx  = num_sizearr_in++;  }
	if (what[1] == IS_STR) { szarray_out_idx = num_sizearr_out++; }
	break;
    }

    if (err) { yyerror("Illegal buffer specification >%s<\n\t\"%s\"",
		       yytextbak,buferrstr[err]); }

    sprintf(&newbufspec[strlen(newbufspec)],"%03d:%d:%d",
	    whatmask,szarray_in_idx,szarray_out_idx);

    sbufspec = (char*)strdup(newbufspec);
}

void reset_func_stuff()
{
    num_parms       = 0;
    ppctype         = 'S';
    assume_type     = 0;
...
char* checked_class_name()
{
#if 0  /* this is only valid if we expect a certain prefix */
    static char *classPrefix = "I";
    int len = strlen(classPrefix);

    /* we make sure that the class_name has the proper layout */
    if ((strncmp(class_name,classPrefix,len) == 0) && (strlen(class_name) > len)) {
	return(&class_name[len]);
    } else {
	yyerror("ClassName <%s> doesn't start with prefix <%s>",
		class_name,classPrefix);
    }
#else
    return(class_name);
#endif
...
void set_ptype()
{
    /* record the <info> collected for this parameter */
    type_mask[num_parms]   = stypemask;
    bufpar_spec[num_parms] = (stypemask & TP_ARRAY) ? sbufspec : NULL;

    /* reset the parameter specific variables */
    stypemask     = 0;
    sbufspec      = NULL;
...
void parerror(const char *err, int par)
{
    yyerror("Parameter #%d: %s\n\tFCT: %s::%s %s\n",
	    par,err,class_name,func_name,sig);
}

void parwarn(const char *err, int par)
{
    yywarn("Parameter #%d: %s\n\tFCT: %s::%s %s\n",
	   par,err,class_name,func_name,sig);
}

void parcheck(int par, int mask)
{
/*
  fprintf(stderr,"Parameter #%d %x: %s\n\tFCT: %s::%s %s\n",
  par,mask,class_name,func_name,sig);

  */

  /* do some sanity checking on the parameters defined here */
    if ( (mask & TP_CONST) && (mask & TP_IO_MASK) ) {
	parerror("const but not __in",par);
    }
    if ( (mask & TP_PR_MASK) && !(mask & TP_IO_MASK) && !(mask & TP_CONST) ) {
	parwarn("ptr or ref parameter as __in might not work properly => consider const",par);
    }
    if ( (mask & TP_IO_MASK) && !(mask & TP_ARRAY) && !(mask & TP_PR_MASK)) {
	parerror("__out parameter must be passed by reference or by pointer",
		 par);
    }
    if ( (mask & TP_ARRAY) && ((mask & TP_PR_MASK) != TP_PTR)) {
	parerror("buffer-parameter (array/string) must be passed by pointer",
		 par);
    }
    if (mask & TP_CALLER) {
	if ((mask & TP_ARRAY) || (mask & TP_IO_MASK)) {
	    parerror("__CALLER_PID in conjunction with IO spec or array spec",par);
	}
	if (caller_index) {
	    parerror("Congratulations: todays winning number is 2.\n"
		     "\tit is hubris to specify two __CALLER_PID parameters",par);
	} else {
	    caller_index = par;
	}
    }
    if (mask & TP_XHANDLE) {
	if ((mask & TP_ARRAY) || (mask & TP_IO_MASK)) {
	    parerror("__XHANDLE in conjunction with IO spec or array spec",par);
	}
	if (xhandle_index) {
	    parerror("Congratulations: todays winning number is 2.\n"
		     "\tit is hubris to specify two __XHANDLE parameters",par);
	} else {
	    xhandle_index = par;
	}
    }
...
{
    if( (in_ppc_call != NO_CALL) && (exported) ) {
	int i;
	int iocnt[3] = { 0, 0 , 0 };
	int ioarrcnt[3] = { 0, 0 , 0 };
	caller_index = 0;
	xhandle_index = 0;
	for( i = 0; i < num_parms; i++ ) {
	    parcheck(i+1,type_mask[i]);
	    if ((type_mask[i] & TP_INOUT) == TP_INOUT) {
		iocnt[1]++;
		if (type_mask[i] & TP_ARRAY) ioarrcnt[1]++;
	    }
	    else if ((type_mask[i] & TP_OUT) == TP_OUT){
		iocnt[2]++;
		if (type_mask[i] & TP_ARRAY) ioarrcnt[2]++;
	    }
	    else if ((type_mask[i] & TP_IN) == TP_IN) {
		iocnt[0]++;
		if (type_mask[i] & TP_ARRAY) ioarrcnt[0]++;
	    }
	}
	sig[strlen(sig)-3]='\0';

	if (parent_name == NULL)
	    yyerror("Exporting class >%s< must have a parent class",
                    class_name);

	printf("%s;%s;%s;%s;%s;%s;%s;%s;%c;%c;%d;%d; %d:%d:%d:%d:%d; %d",
	       checked_class_name(),
	       inherit_name ? inherit_name : parent_name, parent_sig,
	       xaccess_def  ? xaccess_def : "?",
	       xaccess_meth ? xaccess_meth
	       : (xaccess_dflt ? xaccess_dflt : XA_DEFAULT),
	       func_name, type_name, sig,
	       ppctype,in_ppc_call,caller_index,xhandle_index,

	       ioarrcnt[0],ioarrcnt[1],ioarrcnt[2],
	       num_sizearr_in,num_sizearr_out,
	       num_parms);
	printf(";");

	for( i = 0; i < num_parms; i++ ) {
	    int mask = (type_mask[i] & TP_NOARR_MASK) ;
	    if (type_mask[i] & TP_ARRAY)  mask |= 0x0100;
	    if (type_mask[i] & TP_CALLER) mask |= 0x0200;
	    if (type_mask[i] & TP_XHANDLE) mask |= 0x0400;
	    printf(" %03x",mask);
	}
	printf(";");
	for( i = 0; i < num_parms; i++ ) {
	    if (type_mask[i] & TP_ARRAY)
		printf(" %s",bufpar_spec[i]);
	    else
		printf(" ?"); /* don't care */
	}
	printf("\n");
...
void unnamedpar()
{
    ...
    yywarn("Parameter #%d in %s::%s has no argument name\n",
	   num_parms+1,class_name,func_name);
    /* search backward from end of signature for ',' or ')'
     * then insert the name
     */

    len = strlen(sig);
    if (len == 0) {
	/* somehow, in this case we keep on reparsing and calling set_ptype
	 * again, don't ask me why, this is what I observed, so simply
	 * reset the typemask for this purpose.....
	 */
	stypemask = 0;
	type_mask[num_parms] = 0;
	return; /* will not attempt to fix up the signature */
    }
    ptr = sig + len;
    while ((*ptr != ',') && (*ptr != ')')) --ptr;
    sep = *ptr;
    *ptr = '\0';
    sprintf(genname,"__genname_%d %c ",num_parms,sep);
    strcat(ptr,genname);

/*    fprintf(stderr,"%s::%s fix sig >%s<\n",checked_class_name(),func_name,sig);
 */
}
...
/* The following are used in PPC stuff only. */

%token ASYNC	       EXPORTED        INHERIT
%token XACCESS         XACCESSDEF
%token IN              INOUT           OUT
%token INBUF           INOUTBUF        OUTBUF
...
paren_identifier_declarator:
	    ...
	    if( save_ftype ) {
	      ...
	      num_parms = 0;
	      stypemask = 0;
	      func_name = (char *) strdup( last_id );
              sig[strlen(sig) - strlen(func_name) - strlen(" ( ")]='\0';
	      type_name = (char *) strdup( sig );
...
argument_expression_list:
         { not_type = 1; } assignment_expression { not_type = 0; }
         | argument_expression_list              { not_type = 1; }
           ',' assignment_expression             { not_type = 0; }
         ;
...
type_qualifier:
        CONST                   { stypemask |= TP_CONST; }
        | VOLATILE
/*HF*/  | IN                    { stypemask |= TP_IN;    }
/*HF*/  | INOUT                 { stypemask |= TP_INOUT; }
/*HF*/  | OUT                   { stypemask |= TP_OUT;   }
/*HF*/  | INBUF                 { stypemask |= TP_ARRAY | TP_IN;    }
/*HF*/  | INOUTBUF              { stypemask |= TP_ARRAY | TP_INOUT; }
/*HF*/  | OUTBUF                { stypemask |= TP_ARRAY | TP_OUT;   }
/*HF*/  | ASYNC
	  {
	    if( in_ppc_call == NO_CALL )
	      yyerror("PPC specifier must come after 'virtual' keyword");
	    if( ppctype != 'S' )
	      yyerror("Only one PPC type specifier allowed");
            ppctype = 'A';
	  }
/*HF*/  | XACCESS
          {
            xaccess_meth = $1;
	    if( in_ppc_call == NO_CALL )
	      yyerror("XACCESS specifier must come after 'virtual' keyword");
	  }
        ;
...
/*B*/   | declaration_qualifier_list   IDENTIFIER
...
aggregate_name_elaboration:
        aggregate_name derivation_opt inherit_opt access_opt access_dflt '{'
                       { assume_type = 0; public = 0;
		       if (save_parent) {
			   parent_name = $2;
			   inherit_name = $3;
			   xaccess_def = $4;
        ...
        | aggregate_key derivation_opt inherit_opt access_opt access_dflt '{'
                       { assume_type = 0; public = 0;
		       if (save_parent) {
			   parent_name = $2;
			   inherit_name = $3;
...
derivation_list:
	  ...
	  { yyerror("multiple inheritance not yet supported"); }
        ;

parent_class:
                                      global_opt_scope_opt_typedefname
           { $$ = $1; sprintf(parent_sig, "%s", $1); }
        |virtual access_specifier_opt global_opt_scope_opt_typedefname
           { $$ = $3; sprintf(parent_sig, "%s %s %s", $1, $2, $3); }
...
{
	    if( exported ) {
	      if( in_ppc_call != CONSTRUCTOR_CALL )
	        in_ppc_call = STATIC_CALL;
	      save_ftype = 1;
	      sig[0] = '\0';
	    }
	  }
...
access_specifier:
        PUBLIC			{ $$ = strdup("public");    public = 1; exported = 0;}
        | PRIVATE		{ $$ = strdup("private");   public = 0; exported = 0;}
        | PROTECTED		{ $$ = strdup("protected"); public = 0; exported = 0;}
/*HF*/  | EXPORTED                        /* CPP5 make these public */
          { exported = 1; }
        ;
...
member_declaration_list_opt:
        /* nothing */
        | member_declaration_list_opt member_declaration {reset_func_stuff();}
        ;
...
/*B   | VIRTUAL type_specifier                 declarator member_pure_opt */
...
/*B   | VIRTUAL member_conflict_declaring_item */
...
asterisk_or_ampersand:
        '*'
              {
		  if ((stypemask & TP_PTR) && (in_ppc_call != NO_CALL))
		      yyerror("Can't deal with <tp>** on parameter %d %x\n",
			      num_parms+1,stypemask);
		  if (stypemask & TP_REF)
		      stypemask = (stypemask & ~TP_REF) | TP_REFPTR;
		  else
		      stypemask |= TP_PTR;
	      }
        | '&'
              {
		  if ((stypemask & TP_REF) && (in_ppc_call != NO_CALL))
		      yyerror("Can't deal with <tp>&& on parameter %d\n",
			      num_parms+1);
		  if (stypemask & TP_PTR) {
		      stypemask = (stypemask & ~TP_PTR) | TP_PTRREF;
		  } else {
		      stypemask |= TP_REF;
		  }
	      }
...
scope:
        scoping_name CLCL
        { if (!not_type) expect_type = 1; }   /* we must reset to see a potential type */
...
global_or_scoped_typedefname:
        ...
        | global_scope scoped_typedefname	{ $$ = $2; }
        | global_scope TYPEDEFname		{ $$ = $2; }
        ;
...
void yyerror(const char *fmt , ...)
{
    ...
    len = strlen(buf);
    if (buf[len] != '\n') { buf[len] = '\n'; buf[len+1] = '\0'; }
    fprintf(stderr,"CPP5_ERR(L%d): %s", lineno, buf);
...
void yywarn(const char *fmt,...)
{
    ...
    len = strlen(buf);
    if (buf[len] != '\n') { buf[len] = '\n'; buf[len+1] = '\0'; }
    fprintf(stderr,"CPP5_WARN(L%d): %s", lineno, buf);
}
...
main(int argc,char **argv)
{
  ...
  if (argc > 1) {
    ...
    for (i=as; i<argc; i++) {
      if (strcmp(argv[i],"-w") == 0) warnings = 1;
    }
  }
  yyparse();
  return(0);
