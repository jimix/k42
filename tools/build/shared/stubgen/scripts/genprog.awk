...
#
#  $Id: genprog.awk,v 1.85 2004/10/08 21:40:10 jk Exp $
# ############################################################################

BEGIN{
  FS = ";" ;
  iclass = "";
  XF  = dirname "/" "X" filename ".C";
  MF  = dirname "/" "Meta" filename ".C";
  SF  = dirname "/" "Stub" filename ".C";

  xobjdefname = toupper( "X" filename "_DEFC");
  metadefname = toupper( "Meta" filename "_DEFC");
  stubdefname = toupper( "Stub" filename "_DEFC");
  parent_class="";
  xobj_parent="";
  meta_parent="";
  stub_parent="";
...
function createStandardFunc()
{
  ...
  printf("        %s mobj(MetaObj::MAGIC); //tmp obj for vtbl\n",mclass)  > XF;
  ...
  printf("            0,0,theMVTBL,_NUM_METHODS_%s);\n",toupper(mclass))  > XF;
  printf("                                                           \n") > XF;
  printf("    %s *self = new(static_buf) %s;\n",mclass,mclass)		  > XF;
  printf("                                                           \n") > XF;
...
printf("SysStatusXHandle %s::createXHandle(ObjRef iobj, BaseProcessRef pref,\n",mclass)> XF;
  ...
  printf("TYPE_IMPL(%s,%s,%s,1);\n",sclass,stub_parent,nclass)            > SF;
  printf("ObjectHandle %s::__metaoh = { 0, 0 };\n\n",sclass)              > SF;
  printf("void %s::__Reset__metaoh()\n",sclass)                           > SF;
  printf("{ __metaoh.init();\n")                                          > SF;
  printf("  DREFGOBJ(TheTypeMgrRef)->resetTypeHdlr(typeID());\n")         > SF;
  printf("}\n\n")                                                         > SF;
  printf("void %s::__Get__metaoh(ObjectHandle& oh)\n",sclass)             > SF;
  printf("{  if (__metaoh.invalid())\n")                                  > SF;
  printf("    DREFGOBJ(TheTypeMgrRef)->\n")                               > SF;
  printf("      getTypeHdlr(%s::typeID(),__metaoh);\n",sclass)            > SF;
  printf("  oh = __metaoh;\n")                                            > SF;
  printf("}\n\n")                                                         > SF;
}
...
# run through all the functions and create an XObj implementation from
  ...
  printf("#include <xobj/X%s.H> /* auto */\n",filename)                   > XF;
  printf("#include <meta/Meta%s.H> /* auto */\n",filename)                > XF;
  printf("#include \"%s.H\" /* auto */\n",filename)                       > XF;
  printf("\n\n")                                                          > XF;
  ...
  printf("/* FILE IS AUTOMATICALLY GENERATED ==> DO NOT MODIFY */\n\n")   > SF;

  printf("#include <sys/sysIncs.H>\n")                                    > SF;
  printf("#include <sys/ppccore.H>\n")                                    > SF;
  printf("#include <stub/Stub%s.H>\n",filename)                           > SF;
# printf("#include <meta/Meta%s.H>\n",filename)                           > SF;
  printf("\n\n")                                                          > SF;
}
...
#--------------------------------------------------------------
  ...
  if (functype == "C")
    printf("%s::%s%s : %s(StubObj::UNINITIALIZED) {\n",
	   sclass,sclass,stubsig,stub_parent)                             > SF;
  else
    printf("%s\n%s::%s%s\n{\n",rtype,sclass,funcn,stubsig)                > SF;

  declarePPCargs(type,0,SF);

  if (stabmgt_in == 1)
    printf("  uval __len, __alen;\n")                                     > SF;
  if (hasinargs == 1)
    printf("  __inargs_t *__inargs = (__inargs_t*)PPCPAGE_DATA;\n")       > SF;
  if (hasoutargs == 1)
    printf("  __outargs_t *__outargs = (__outargs_t*)PPCPAGE_DATA;\n")    > SF;
  if (stabmgt_in > 0)
    printf("  char *__stab = PPCPAGE_DATA + sizeof(__inargs_t);\n")       > SF;

  printf("  SysStatus __ppc_rc;\n")                                       > SF;

  if (functype != "V") {
    printf("  if (__metaoh.invalid()) {\n")                               > SF;
    printf("    __ppc_rc = DREFGOBJ(TheTypeMgrRef)->\n")                  > SF;
    printf("      getTypeHdlr(%s::typeID(),__metaoh);\n",sclass)          > SF;
    if (functype == "C") {
      # FIXME: there's no way to return failure from a constructor
      printf("    if (_FAILURE(__ppc_rc)) return;\n")                     > SF;
    } else {
      printf("    _IF_FAILURE_RET(__ppc_rc);\n")                          > SF;
    }
    printf("  }\n")                                                       > SF;
  }
  if (hasinargs == 1)
    printf("  SET_PPC_LENGTH_CH(sizeof(__inargs_t));\n")                  > SF;
  printf("\n  /* STABMGT = %d %d */\n",stabmgt_in,stabmgt_out)            > SF;

  #---------------------------------------------------------------------------
  # first copy regular args, all arrays and strings are dealt with separately
  #---------------------------------------------------------------------------

  for (j=1;j<=nump;j++) {
    if ((inoutspec[j] <= 1)) {
      if ((isarray[j] == 0) && (j != calleridx) && (j != xhandleidx)) {
	printf("  __inargs->%s = %s%s; /* %d */\n",
	       nameset[j],dref[j],nameset[j],prspec[j])                   > SF;
      }
    }
  }
  ...
  if (stabmgt_in == 1) {
    ...
    cnt_vtab_args = 0;
    if (require_vtablen_in == 1)
      printf("  __inargs->__vtablen = 0;\n")                              > SF;
    for (j=1;j<=nump;j++) {
      if ((inoutspec[j] == 0) && (isarray[j] == 1)) {
	if      (a_spec_in[j] == IS_STR_ARRAY) {
	  printf("  SGSU_PUT_STR(%s,%d);\n",nameset[j],a_stabin_idx[j])   > SF;
	}
	else if (a_spec_in[j] == IS_CONST_ARRAY) {
	  printf("  SGSU_PUT_FARRAY(%s,%s);\n",nameset[j],a_inlength[j])  > SF;
	}
	else if (a_spec_in[j] == IS_VAR_ARRAY) {
	  printf("  SGSU_PUT_VARRAY(%s,%s%s);\n",nameset[j],
		 dref[a_in_var_idx[j]],a_inlength[j])                     > SF;
	}
      }
      cnt_vtab_args++;
    }
	...
	if      (a_spec_in[j] == IS_STR_ARRAY) {
	  printf("  SGSN_PUT_STR(%s,%d);\n",nameset[j],a_stabin_idx[j])   > SF;
	}
	else if (a_spec_in[j] == IS_CONST_ARRAY) {
	  printf("  SGSN_PUT_FARRAY(%s,%s);\n",nameset[j],a_inlength[j])  > SF;
	}
	else if (a_spec_in[j] == IS_VAR_ARRAY) {
	  printf("  SGSN_PUT_VARRAY(%s,%s%s);\n",nameset[j],
		 dref[a_in_var_idx[j]],a_inlength[j])                     > SF;
	}
      }
    }
  ...
  if      (ppctype == "S") printf("  PPC_CALL(__ppc_rc,")                 > SF;
  else if (ppctype == "A") printf("  PPC_ASYNC(__ppc_rc,")                > SF;
  else error(200, "Unknown PPC type")
  if (functype == "V") printf("this->oh.commID(),this->oh.xhandle()")     > SF;
  else                 printf("__metaoh.commID(),__metaoh.xhandle()")     > SF;
  printf(",%d);\n",vindex)                                                > SF;

  #---------------------------------------------------
  # return copy any results
  #---------------------------------------------------

  if (hasoutargs == 1)
    printf("  if (__ppc_rc >= 0) {\n")                                    > SF;
  ...
  if (stabmgt_out == 1) {
	...
	if      (a_spec_out[j] == IS_STR_ARRAY) {
	  printf("    SGS_GET_STR(%s,%d);\n",nameset[j],a_stabout_idx[j]) > SF;
	}
	else if (a_spec_out[j] == IS_CONST_ARRAY) {
	  printf("    SGS_GET_FARRAY(%s,%s);\n",nameset[j],a_outlength[j])> SF;
	}
	else if (a_spec_out[j] == IS_VAR_ARRAY) {
	  if (a_outlength[j] == "__rc") outlenpref = "__ppc_rc";
	  else                          outlenpref = "__outargs->"a_outlength[j];
	  printf("    SGS_GET_VARRAY(%s,%s);\n",nameset[j],outlenpref)> SF;
	}
      }
    }
  } # done with variable length managment

  #-------------------------------------------------------------------------
  # now copy regular out parameter
  #-------------------------------------------------------------------------

  for (j=1;j<=nump;j++)
    if ((inoutspec[j] >= 1) && (isarray[j] == 0))
      printf("    %s%s = __outargs->%s;\n",dref[j],nameset[j],nameset[j]) > SF;

  #-------------------------------------------------------------------------
  # Done copying results
  #-------------------------------------------------------------------------
  ...
  if (hasinargs == 1) {
    ...
    printf("  RESET_PPC_OVERFLOW();\n")                                   > SF;
    if (functype != "C")
      printf("  return(_SERROR(0,2,ENOSPC));\n")                           > SF;
  }
  printf("}\n\n")                                                         > SF;
