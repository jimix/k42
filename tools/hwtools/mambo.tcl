...
#
#  $Id: mambo.tcl,v 1.3 2004/12/30 23:54:55 mostrows Exp $
# ############################################################################
...
# proc k42_start_proc, then this procedure is invoked instead of instantiating
# a simulator.  The procedure is invoked after k42.tcl and utils.tcl are
# sourced.  
#
...
if { [ info exists env(MAMBO_NUM_CPUS) ] } {
    mysim config cpus $env(MAMBO_NUM_CPUS)
}

if { [ info exists env(MAMBO_ROM_FILE) ] } {
    #mysim config ROM_file $env(MAMBO_ROM_FILE)
    mysim mcm 0 config  ROM_file $env(MAMBO_ROM_FILE)
}


if { ! [info exists mambo_no_memory_config] } {
  if { [catch [mysim config memory "$env(MAMBO_MEM)M"]] } {}
  if { [catch [mysim config memory_file $env(MAMBO_GARB_FNAME)]] } {}
}
...
if { [info exists env(MAMBO_DISK_0)] } {
    puts "Configure $env(MAMBO_DISK_0) as /dev/mambobd0"
    mysim bogus disk init 0 $env(MAMBO_DISK_0) rw
}
